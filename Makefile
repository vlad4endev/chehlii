# ЧехлИИ — команды локальной разработки (нативный запуск, без Docker).
...
help:
	...
	@echo "db        — создать роль и базу chehlii"
...
db:
	$(PG_BIN)/psql -d postgres -tc "SELECT 1 FROM pg_roles WHERE rolname='chehlii'" | grep -q 1 || \
		$(PG_BIN)/psql -d postgres -c "CREATE ROLE chehlii LOGIN PASSWORD 'chehlii';"
	$(PG_BIN)/psql -d postgres -tc "SELECT 1 FROM pg_database WHERE datname='chehlii'" | grep -q 1 || \
		$(PG_BIN)/createdb -O chehlii chehlii
	@echo "OK: роль и база chehlii готовы"
...
reset-db:
	$(PG_BIN)/dropdb --if-exists chehlii
	$(PG_BIN)/createdb -O chehlii chehlii
	$(MAKE) migrate seed
