...
async def yandex_status(order_id: int, session: Session) -> dict:
    ...
    «Отправлен»/«Доставлен» делаем здесь же.

    ponytail: планировщика в проекте нет — статус двигается только когда эндпоинт
    дёрнули. Понадобится автоматика — вызывать по cron/воркеру для заказов со
    статусом «Отправлен» и заполненным tracking_code.
    """
